 ...
 * 配置来源优先级：
 * 1) src/plugins/aisummary.config.js 顶部注释中的 AI_SUMMARY_API / AI_SUMMARY_KEY
...
/**
 * 调用外部 API 生成摘要；优先读取 aisummary.config.js 注释配置，其次读取环境变量。
...
/**
 * 主流程：扫描文章、生成摘要并写入文件。
 * 规则：若 src/plugins/aisummary.config.js 未声明 AI_SUMMARY_API，则保持 summary 为空值
